//! referee-server 集成测试 — 多实例管理 / 对话往返 / 崩溃恢复 / TCP 传输
//!
//! 覆盖 REFEREE_SERVER_IMPL.md §10 的 13 个用例。全部使用 MockProvider 直连，
...
fn temp_dir(tag: &str) -> PathBuf {
    std::env::temp_dir().join(format!("referee_server_test_{tag}_{}", uuid::Uuid::new_v4()))
}
